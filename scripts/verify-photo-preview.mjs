...
try {
  const created = await context.request.post(`${baseURL}/api/imports/`, { data: {} });
  if (!created.ok()) throw new Error(`Draft creation failed with ${created.status()}.`);
  let draft = await created.json();
  draftID = draft.id;
  const uploaded = await context.request.post(`${baseURL}/api/imports/${draft.id}/sources`, {
    multipart: {
      revision: String(draft.revision),
      file: {
        name: 'noted-photo-20mp.jpg',
        mimeType: 'image/jpeg',
        buffer: await readFile(join(repository, 'testdata/fixtures/noted-photo-20mp.jpg')),
      },
    },
  });
  if (!uploaded.ok()) throw new Error(`Fixture upload failed with ${uploaded.status()}.`);
  ...
  const result = await page.evaluate(async (current) => {
    ...
    const sourceResponse = await fetch(`/api/imports/${current.id}/sources/${source.id}`);
    const sourceBitmap = await createImageBitmap(await sourceResponse.blob());
    const sourceScale = Math.min(1, 1050 / Math.max(sourceBitmap.width, sourceBitmap.height));
    ...
    sourceCanvas.getContext('2d').drawImage(sourceBitmap, 0, 0, sourceCanvas.width, sourceCanvas.height);
    sourceBitmap.close();
    ...
    const run = (payload) =>
      new Promise((resolve, reject) => {
        ...
        worker.onerror = () => reject(new Error('The processing worker failed to start.'));
    ...
    try {
      ...
      const render = async (pageNumber) => {
        ...
        const viewport = pdfPage.getViewport({ scale: 1050 / Math.max(base.width, base.height) });
      ...
      for (let index = 0; index < finalPixels.length; index += 4) {
        ...
        brightness += gray;
        if (gray < 120) dark++;
        if (gray >= 145 && gray < 230 && Math.max(r, g, b) - Math.min(r, g, b) < 15) faint++;
  ...
  const failures = [];
  if (result.pages !== 2) failures.push(`expected 2 ordered pages, received ${result.pages}`);
  ...
  if (result.meanDifference > 24) failures.push('preview and final pixels differ too much');
  if (result.meanBrightness < 225) failures.push('paper cleanup did not lighten the page');
  ...
  if (draftID) await context.request.delete(`${baseURL}/api/imports/${draftID}/`);
  await context.close();
  await browser.close();
}
