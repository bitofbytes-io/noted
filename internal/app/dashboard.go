...
func (s *Service) recentImports(ctx context.Context, userID string) ([]Asset, error) {
	rows, err := s.Pool.Query(ctx, `
		SELECT a.id::text,a.edition_id::text,a.asset_type,a.original_filename,COALESCE(a.display_name,a.original_filename),a.media_type,a.byte_size,a.sha256,COALESCE(a.source_url,''),a.rights_note,a.playback_capable,a.playback_validation_status,a.playback_validation_issues,a.created_at
	...
	for rows.Next() {
		var item Asset
		if err := rows.Scan(&item.ID, &item.EditionID, &item.AssetType, &item.OriginalFilename, &item.DisplayName, &item.MediaType, &item.ByteSize, &item.SHA256, &item.SourceURL, &item.RightsNote, &item.PlaybackCapable, &item.PlaybackValidation.Status, &item.PlaybackValidation.Issues, &item.CreatedAt); err != nil {
			return nil, err
		}
		setAssetURLs(&item)
