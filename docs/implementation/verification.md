...
| POC-030–037 | Edition-bound upload, content sniffing, size limits, provenance/checksum metadata, storage interface, opaque keys | `TestDetectUploadValidatesExtensionAndContent`, store tests, upload-cleanup and ownership integration tests; Flow B E2E |
...
| POC-070–077 | Explicit durable timer, one-running-timer database constraint, timed/manual edit/delete fields, aggregate refresh | timer service test, practice/domain and weekly aggregation Go tests, Flow C E2E |
...
| Command | Result |
|---|---|
| `make test` | Passed: all Go packages including PostgreSQL integration tests; 6 Angular test files / 7 tests |
| `make lint` | Passed: gofmt check, `go vet ./...`, Prettier check, TypeScript no-emit check |
| `make build` | Passed: all Go packages and Angular production build; initial UI bundle 273.65 kB raw / 76.46 kB estimated transfer, score engines lazy-loaded |
| `make test-e2e` | Passed: 6/6 tests covering Flows A–C in installed desktop Chrome and WebKit at 1024×1366 portrait |
| `npm audit` | Passed: 0 known vulnerabilities in the pinned dependency tree |

Backend coverage specifically includes domain/range validation, upload extension/content matching, opaque path safety, object round trips, ownership scoping, asset authorization, cleanup after metadata failure, UTC daily aggregation, Monday-first weeks, and fail-closed development authentication.

## Browser verification

- Desktop Chrome at 1440×1000: dashboard and details inspected; PDF rendered through its authenticated API stream; MusicXML rendered eight measures; the bundled SoundFont loaded; clicking Play changed the control to Pause; measures 2–4 and looping stayed active.
- Portrait responsive pass at 1024×1366: dashboard, work details, PDF reader, and score player inspected with no document-width overflow. Playwright's WebKit project repeated all three core flows with touch/mobile settings.
...
A detached worktree at committed revision `e8ad47b` was created with no dependency, build, configuration, or asset output. In that worktree:

1. `make setup` created `.env` and the local asset tree, downloaded Go modules, and completed `npm ci` with 0 vulnerabilities.
2. `make test`, `make lint`, and `make build` all passed independently of the original working directory.
3. `make local` reused the named local PostgreSQL Compose project, migrated and seeded it, started both services, returned `{"status":"ready"}` from the API, served the Angular index, and proxied the seeded dashboard response through the UI server.
