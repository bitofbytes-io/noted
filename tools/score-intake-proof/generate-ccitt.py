"""Original Noted CC0 fixture, proof-local single-strip CCITT encoding."""
...
tiff_data = BytesIO()
image.save(tiff_data, format="TIFF", compression="group4", tiffinfo={278: HEIGHT})
tiff_data.seek(0)
...
assert strip_count == 1, "PDF CCITT stream must contain one independently encoded strip"
...
destination = Path(sys.argv[1]) / "valid-ccitt-original.pdf"
image.convert("RGB").save(destination.with_name("valid-ccitt-reference.png"))
destination.with_name("valid-ccitt-encoding.json").write_text(json.dumps({"stripCount":strip_count,"rowsPerStrip":tiff.tag_v2[278],"width":WIDTH,"height":HEIGHT}))
